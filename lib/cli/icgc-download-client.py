...
parser.add_argument('--output_dir', default='/icgc/mnt/downloads',  help='Directory to store downloaded files')
...
if args.repo == 'ega':
    ...
    if args.manifest:
        icgc_script.icgc_manifest_call(args.file_id, config['access.icgc'],
                                       config['tool.icgc'], args.output_dir)
    else:
    ...
    if args.manifest:
        gdc_script.gdc_manifest_call(args.file_id, config['access.gdc'], config['tool.gdc'], args.output_dir,
                                     config['udt'])
    else:
